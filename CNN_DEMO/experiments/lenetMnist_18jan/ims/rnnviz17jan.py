"""
Creates visualizations for RNN hyperparameter search results.
...
#    if expID=='amAdam5_me1':
      ...
      ar = rez[0].args
      # Model Size.
      hidSize = str(ar['rnnHidSize'])

      # Alg name.
      algName = ar['opt_method']
      TITLE  = 'RNN 1-'+hidSize +'; trained with '+algName
...
hidSize = str(100)#str(rez[0].args['rnnHidSize'])

# (0.) Each method gets a unique color.
colors  = {"sgd":'b', "adam":'k',   # Backprop-based methods
            "amAdam5_me1":'r',      # 5 subproblem iter, 1 miniepoch.
            "amAdam5_me2":'r',      # 5 subproblem iter, 2 miniepoch.
            "amAdam5_me3":'r',      # 5 subproblem iter, 3 miniepoch.
            "amAdam1_me1":'y',      # 1 subproblem iter, 1 miniepoch.
            "amAdam1_me2":'y',      # 1 subproblem iter, 2 miniepoch.
            "amAdam1_me5":'y',      # 1 subproblem iter, 5 miniepoch.
            "amAdam20_me1":'m',     # 20 subproblem iter (codes only), 1 miniepoch.
            "amAdam15_me2":'m',     # 1 subproblem iter (codes only), 2 miniepoch.
            "amAdam15_me3":'m',     # 15 subproblem iter (codes only), 3 miniepoch.
            "amAdam15_me5":'m',     # 15 subproblem iter (codes only), 5 miniepoch.
          }
markers = {"sgd":'o', "adam":'o',   # Backprop-based methods
            "amAdam5_me1":'o',      # 5 subproblem iter, 1 miniepoch.
            "amAdam1_me1":'o',      # 1 subproblem iter, 1 miniepoch.
            "amAdam20_me1":'o',     # 20 subproblem iter (codes only), 1 miniepoch.
            "amAdam5_me2":'*',      # 5 subproblem iter, 2 miniepoch.
            "amAdam1_me2":'*',      # 1 subproblem iter, 2 miniepoch.
            "amAdam15_me2":'*',     # 1 subproblem iter (codes only), 2 miniepoch.
            "amAdam5_me3":'^',      # 5 subproblem iter, 3 miniepoch.
            "amAdam15_me3":'^',     # 15 subproblem iter (codes only), 3 miniepoch.
            "amAdam1_me5":'_',      # 1 subproblem iter, 5 miniepoch.
...
markerSize = 4
resUsableTitle = 'no-skip RNN: 1-'+hidSize
numEpochs=10
######## ALL PLOTS
for i, xLims in enumerate([[-0.1, numEpochs+0.1]]):
  for k,yLims in enumerate([[0.05, 0.5]]):
    plt.figure(i*k + k)
    plt.clf()
    colorID=0
...
#                      label=plotNames[expID]+' Train')
      plt.errorbar(rez['xax'], rez['meansTe'], rez['stdsTe'], 
                      ...
                      markersize = markerSize, linewidth = 1,
                      label=expID+' Test (N={})'.format(rez['numTr']))
    plt.xlim(xLims)
    plt.ylim(yLims)
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    ...
    plt.savefig(savePath+'ALL_METHODS_best_jan2_1-'+str(hidSize)+'lim'+str(i)+str(k)+'.png', bbox_inches='tight')


######## RELEVANT PLOTS
keepThese = ['amAdam5_me1','amAdam1_me1', 'amAdam1_me2','sgd', 'adam',]
for i, xLims in enumerate([[-0.1, numEpochs+0.1]]):
  for k,yLims in enumerate([[0.05, 0.5]]):
    plt.figure(i*k + k)
    plt.clf()
    ...
    for expID, rez in bestRez.items():
  #    print('expID = '+expID+': '+str(rez['stdsTr']))
      if rez['acc']==-1 or not (expID in keepThese):
        continue 
      # Else, add to the axes.
#      plt.errorbar(rez['xax'], rez['meansTr'], rez['stdsTr'], 
#                      marker = trmark, color=colors[expID],
...
#                      label=plotNames[expID]+' Train')
    ...
    # Get the legend off of the main plot:
    plt.legend(loc=9, bbox_to_anchor=(0.5, -0.1), ncol=3)
    plt.savefig(savePath+'RELEVANT_best_jan2_1-'+str(hidSize)+'lim'+str(i)+str(k)+'.png', bbox_inches='tight')
