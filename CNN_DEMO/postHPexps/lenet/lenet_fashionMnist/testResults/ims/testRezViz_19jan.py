...
#                    label=names[expID]+' TR')
    ...
    plt.annotate('%0.3f' % yvals[-1], xy=(1,yvals[-1]*xtras[expID]), xytext=(8,0),
...
xtras={'adam':1.02, 'amAdam1_me1':1, 'sgd':0.999}
                        #fmnist0   #mnist0  
for i,xlims in enumerate([None, [1,50]]):
  for k,lims in enumerate([[0.1,0.85],[0.1,1]]):
    ...
    for expID, rez in avgRez.items():
      ...
      plt.errorbar(rez['xax0'], yvals, rez['stdsR0'], 
                      ...
                      label=names[expID]+' (N={})'.format(rez['numTr']))
      # Plot the final accuracy to the right of the axes.
      plt.annotate('%0.3f' % yvals[END], xy=(1,yvals[END]*xtras[expID]), xytext=(8,0),
                    xycoords=('axes fraction', 'data'), textcoords='offset points')
    # Now plot a marker designating when TESTING phase began.
  #  plt.axvline(x=59.5, linewidth=2, label='Begin testing')
    if xlims is None:
    ...
    plt.title('Epoch 0 Test Accuracy\n'
               + resUsableTitle)
    plt.legend()#loc=9, bbox_to_anchor=(0.5, -0.1), ncol=3)
    plt.savefig(savePath+'bestMiniBatches_19jan-'+modelName+datName+'lim'+str(i)+str(k)+'.png', bbox_inches='tight')
...
#                    label=names[expID]+' TR')
  ...
  ax1.errorbar(rez['xax'], yvals, rez['stdsTe'], 
                  ...
                  label=names[expID]+' (N={})'.format(rez['numTr']))
  # Plot the final accuracy to the right of the axes.
  ax1.annotate('%0.3f' % yvals[-1], xy=(1,yvals[-1]*xtras[expID]), xytext=(8,0),
                xycoords=('axes fraction', 'data'), textcoords='offset points')
...
ax1.legend(loc='lower left')#loc=9, bbox_to_anchor=(0.5, -0.1), ncol=3)

###########################################
## Now do epoch-0 inside the above axis...
xtras={'adam':1.02, 'amAdam1_me1':1, 'sgd':0.999}

ax2=plt.axes([.45, .11, .45, .4])
...
for expID, rez in avgRez.items():
  ...
  ax2.errorbar(rez['xax0'], yvals, rez['stdsR0'], 
                  ...
                  label=names[expID]+' (N={})'.format(rez['numTr']))
  # Plot the final accuracy to the right of the axes.
  ax2.annotate('%0.3f' % yvals[END], xy=(1,yvals[END]*xtras[expID]), xytext=(8,0),
                xycoords=('axes fraction', 'data'), textcoords='offset points')
ax2.set_xlim([min(rez['xax0'])-xtraAx2, END+xtraAx2])
ax2.set_xticks([])
ax2.set_ylim(ylim2)
#ax2.set_xlabel('Minibatches')
ax2.annotate('Epoch 0 Minibatches', xy=(12,0.79))
#ax2.ylabel('Accuracy')
